...
import { Card, Button, UncontrolledCollapse, CardBody, Badge } from "reactstrap";
...
const ProductFilters = ({
    ...
    const options = [
        ...
        { value: "mostViewed", label: "Sản phẩm mua nhiều" },
    ...
    useEffect(() => {
        ...
        if (categoryNameSlug) {
            const selectedCategory = categories.find(category => category.name_slug === categoryNameSlug);
            if (selectedCategory) {
                handleSelectedCategory(selectedCategory);
            }
        }
        if (brandNameSlug) {
            console.log(brandNameSlug);
            handleResetFilters();
            const selectedBrand = brands.find(brand => brand.name_slug === brandNameSlug);
            if (selectedBrand) {
                handleSelectedBrand(selectedBrand.name,true);
            }
        }
    ...
    }, [products, categories, brands, minProductPrice, maxProductPrice, promotions,categoryNameSlug,promotionId,brandNameSlug]);
    ...
    useEffect(() => {
        ...
        filteredProducts = filteredProducts.filter(product => {
            const ratingCheck = selectedRatings.length === 0 || selectedRatings.includes(Math.round(product.averageRating));
        ...
        setProductList(filteredProducts);
        // eslint-disable-next-line
    }, [selectedRatings, selectedCategory, selectedBrands, selectedPromotions, sliderValues, selectedDiscounts, selectedOption]);  // Note addition of selectedOption in the dependency array

    // useEffect(() => {
    //     console.log("Danh sách lọc", productList);
    // }, [productList])
                    ...
                    <div>
                        ...
                        <ul className="list-unstyled mb-0 filter-list">
                            <li className="filter-category">
                                <Badge className="badge-gradient-primary"> Bé trai</Badge>
                                <ul className="list-unstyled mb-0">
                                    {categories
                                        .filter(category => category.name.toLowerCase().includes("trai"))
                                        .map((category) => (
                                            <li key={category._id}>
                                                <Link
                                                    to="#"
                                                    className={`d-flex py-1 align-items-center ${cate === category.name ? "active" : ""}`}
                            ...
                            <li className="filter-category">
                                <Badge className="badge-gradient-danger">Bé gái</Badge>
                                <ul className="list-unstyled mb-0">
                                    {categories
                                        .filter(category => category.name.toLowerCase().includes("gái"))
                                        .map((category) => (
                                            <li key={category._id}>
                                                <Link
                                                    to="#"
                            ...
                            <li className="filter-category">
                                <Badge className="badge-gradient-dark">Phụ kiện</Badge>
                                <ul className="list-unstyled mb-0">
                                    {categories
                                        .filter(category => !(category.name.toLowerCase().includes("trai")) && !(category.name.toLowerCase().includes("gái")))
                                        .map((category) => (
                                            <li key={category._id}>
                                                <Link
                                                    to="#"
                                    ...
                                    {['Hơn 50%', 40, 30, 20, 10, 'Dưới 10%'].map((discount, index) => (
                                        <div className="form-check" key={`discount-${index}`}>
                                            ...
                                            <label className="form-check-label" htmlFor={`productdiscountRadio${index}`}>
                                                {typeof discount === 'number' ? `${discount}% đến ${discount + 9}%` : discount}
                                            </label>
                                        </div>
                                    ))}
